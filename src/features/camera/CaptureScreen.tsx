...
const styles = stylex.create({
  ...
  // A whole outfit stands head to toe, so its guide is a tall rounded
  // shape rather than the garment's inset rectangle.
  outfitGuide: { insetBlock: '5%', insetInline: '28%', borderRadius: '999px' },
  banner: {
    backgroundColor: tokens.dark,
    color: tokens.darkFg,
    borderRadius: tokens.radiusMedia,
...
/**
 * Framing a garment or an outfit, or choosing a photo of one.
 *
...
export function CaptureScreen({
  ...
  handEntry: string;
  /**
   * Show the hand-entry link while the camera is live, not only in low
  ...
  const panel = status === 'opening' || status === 'live' ? null : copy.panels[status];
  ...
  const handEntryLink = (
    <Link to={handEntry} {...stylex.props(styles.link)}>
      {copy.handEntry}
    </Link>
  );
